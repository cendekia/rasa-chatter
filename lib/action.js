...
async function generate() {
    ...
    try {
        ...
        spinner.info('🤖  Your command was');
        log(normal('     ' + `python3 -m chatette ${env.dsl_file} -f`));

        if (stderr) { 
            spinner.fail('🤖  '+stderr);
        } else {
            spinner.info(stdout);
            spinner.succeed('Successcully generate it');
        }
    } catch (error) {
        spinner.info('🤖  Your command was');
...
async function convert() {
    ...
    try {
        ...
        spinner.info('🤖  Your command was');
        log(normal('     ' + `rasa data convert nlu --data ${env.output_file} --out ${env.nlu_file} --format ${defaultRasaFormat}`));
